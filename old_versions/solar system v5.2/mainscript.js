var canvas = document.querySelector('canvas'),
	...
	planets = [
        new Planet('Sun', 0, 0, 0, 50, '#FAF71C'), // (velocity, angle, distance, size, color)
		new Planet('Mercury', 47.36 /*Math.random() * 5 - 5*/, (Math.random() * 2) * Math.PI , 116, 4.8, '#787677'),
		new Planet('Venus', 35.02 /*Math.random() * 5 - 5*/, (Math.random() * 2) * Math.PI, 216, 12.1, '#C29C5E'),
		new Planet('Earth', 29.78 /*Math.random() * 5 - 5*/, (Math.random() * 2) * Math.PI, 300, 12.7, '#2EFEF7'),
		new Planet('Mars', 24.13 /*Math.random() * 5 - 5*/, (Math.random() * 2) * Math.PI, 396, 6.76, '#991400'),
	],
	maxParticles = 30000,
	emissionRate = 3
...
setInterval(function() {
    ...
    for (var i = 0; i < planets.length; i++) {
        if (planetsShow[i]) {
            ctx.rect(planets[i].position.x + planets[i].size , planets[i].position.y - planets[i].size - 60, 150, 60);
...
function mouseMoved(e) {
    for (var i = 0; i < planets.length; i++) {
        if ((e.clientX <= (planets[i].position.x + planets[i].size)) && (e.clientX >= (planets[i].position.x - planets[i].size)) && ((e.clientY - 20) <= (planets[i].position.y + planets[i].size)) && ((e.clientY - 20) >= (planets[i].position.y - planets[i].size))) {
            console.log(planets[i].name)
            planetsShow[i] = true

            //ctx.rect(planets[i].position.x, planets[i].position.y, 150, 100);
            //ctx.strokeStyle = "white";
            //ctx.stroke();
        }
        else {
        ...
        //console.log(planets[0].position.x, planets[0].position.y)
    }
...
function Planet(name, velocity, angle, distance, size, color) {
	//this.position = point
	this.velocity = velocity
	this.mass = size * 10;
	this.angle = angle
