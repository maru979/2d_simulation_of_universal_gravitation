var canvas = document.querySelector('canvas'),
	...
	planets = [
        new Planet(0, 0, 0, 50, 'yellow'), // (velocity, angle, distance, size, color)
		new Planet(Math.random() * 5 - 5, (Math.random() * 2) * Math.PI , 120, 10, '#009900'),
		new Planet(Math.random() * 5 - 5, (Math.random() * 2) * Math.PI , 250, 15, 'blue'),
		new Planet(Math.random() * 5 - 5, (Math.random() * 2) * Math.PI , 350, 20, '#470b00'),
	],
	maxParticles = 30000,
	emissionRate = 3
...
function PlanetMove(){
	for (var i = 0; i < planets.length; i++) {
		...
		planet.position.y = planets[0].position.y + Math.sin(planet.angle) * planet.distance;
