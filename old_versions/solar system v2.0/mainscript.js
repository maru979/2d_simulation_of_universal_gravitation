var canvas = document.querySelector('canvas'),
	...
	fields = [
		 new Field(new Vector(680, 300), 500),
	...
	planets = [
		new Planet(new Vector(500, 300), 2, 500, Math.PI/2),
		//new Planet(new Vector(350, 300), 2, 500, Math.PI/2)
	],
	maxParticles = 30000,
	emissionRate = 3
...
function createParticles() { //grid particles
...
function PlanetMove(){
	for (var i = 0; i < planets.length; i++) {
		...
		//planet.position.x = fiels[0].position.x + Math.cos(planet.angle) * 180;
		//planet.position.y = fiels[0].position.y + Math.sin(planet.angle) * 180;
		planet.angle += planet.velocity/100;	
	}
}
...
function plotParticles(boundsX, boundsY) {
	...
	for (var i = 0; i < particles.length; i++) {
		...
		for (var j = 0; j < fields.length; j++) {
			if (Math.abs(pos.x - fields[j].position.x)<=8 && Math.abs(pos.y - fields[j].position.y)<=8) {
				skip = true
				break
			}
			if (Math.abs(pos.x - planets[j].position.x)<=8 && Math.abs(pos.y - planets[j].position.y)<=8) {
				skip = true
				break
			}
		}
...
function drawCircle(object) {
	ctx.fillStyle = object.drawColor
	ctx.beginPath()
	ctx.arc(object.position.x, object.position.y, planetSize, 0, Math.PI*2)
	ctx.closePath()
	ctx.fill()
}
...
function Planet (point, velocity, mass, angle){
	this.position = point
	this.velocity = velocity;
	this.mass = mass 
	this.angle = angle
	this.drawColor = '#009900'
}
